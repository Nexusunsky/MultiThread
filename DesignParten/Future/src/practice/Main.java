...
import practice.content.Content;
import practice.content.Retriever;
...
public class Main {
    ...
    private static void saveToFile(String filename, Content content) {
        ...
        try {
            ...
            System.out.println("FileOutputStream write exception" + e.getMessage());
        } finally {
            if (out != null) {
                try {
                    out.close();
                } catch (IOException e) {
                    System.out.println("FileOutputStream close exception" + e.getMessage());
                }
            }
        }
    }
